...
@interface SelectWeight ()<UITextFieldDelegate,UIPickerViewDataSource,UIPickerViewDelegate>
{
...
- (NSMutableArray *)weightArray {
    ...
    if (_weightArray == nil) {
        ...
        //区间
        for (int weight = 0; weight <= 250; weight++) {
            NSString *str = [NSString stringWithFormat:@"%dkg", weight];
            [_weightArray addObject:str];
        }
    }
    return _weightArray;
}
...
- (instancetype)init
{
    ...
    if (self) {
        ...
        //picker默认值
        [_pickerView selectRow:weightIndex + 70 inComponent:0 animated:YES];
