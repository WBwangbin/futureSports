...
+ (id)sharedInstance
{
    ...
    @synchronized (self)
    {
        if (log == nil)
        {
            ...
            // 初始化DDLog日志输出，在这里，我们仅仅希望把日志语句发送至文件
            ...
            fileLogger.maximumFileSize = 5 * 1024 * 1024; //设置日志大小为5M
            [DDLog addLogger:fileLogger];
            logFilePath = fileLogger.logFileManager.logsDirectory;
            // 2.2打印日志文件目录
            NSLog(@"Print log file directory:%@", logFilePath);
        }
    }
...
- (NSString *)compressLogFiles
{
    ...
    if (!isOK)
    {
        NSLog (@"create log zip file fail.");
        return nil;
    }

    for (NSString *fileName in files)
    {
        if (![fileName containsString:@"未来体育"])
        {
            continue;
        }
        ...
        if (!isOK)
        {
            NSLog (@"add log file[file name:%@] to zip fail.", fileName);
            continue;
        }
    }
...
- (NSString *)getCompressLogFileName
{
    ...
    NSString *compressedLogFileName =
    [NSString stringWithFormat:@"%@_%@.zip", @"ios_speedpro", [SVTimeUtil currentTimeStamp]];
...
+ (void)clearAllLog
{
    ...
    for (NSString *fileName in files)
    {
        ...
        if (error)
        {
            NSLog (@"delete log file fail. file path:%@,  error:%@", filePath, error);
        }
        else
        {
            NSLog (@"delete log file success. file path:%@", filePath);
        }
    }

    NSLog (@"finish to clear log file.");
}
